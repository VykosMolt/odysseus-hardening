...
def setup_gallery_routes() -> APIRouter:
    ...
    async def gallery_ai_upscale(request: Request):
        ...
        db = SessionLocal()
        try:
            q = db.query(ModelEndpoint).filter(ModelEndpoint.model_type == "image", ModelEndpoint.is_enabled == True)
            if user:
        ...
        if not base:
            ...
            try:
                ...
                if matched_ep is not None:
                    base = (matched_ep.base_url or base).rstrip("/")
                    api_key = matched_ep.api_key
